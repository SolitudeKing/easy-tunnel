...
from ..model.tunnel import TunnelConfig, TunnelState
from ..utils.shell import powershell_join as _powershell_join
...
class SSHTunnelService:
    def __init__(
        self, ssh_executable: str | None = None, startup_timeout: float = 12.0
    ) -> None:
    ...
    def snapshots(self) -> list[RuntimeSnapshot]:
        with self._lock:
            return [
                self._snapshot(self._items[item])
    ...
    def build_command(self, config: TunnelConfig) -> list[str]:
        if not self.ssh_executable:
            raise RuntimeError(
                "未找到 OpenSSH 客户端，请先安装或启用系统 OpenSSH Client"
            )
    ...
    def start(self, tunnel_id: str) -> bool:
        with self._lock:
            ...
            if not self.ssh_executable:
                self._set_error(
                    runtime,
                    "未找到 OpenSSH 客户端，请在 Windows 可选功能中安装 OpenSSH Client",
                )
                return False
        ...
        threading.Thread(
            target=self._read_output, args=(tunnel_id, process), daemon=True
        ).start()
        threading.Thread(
            target=self._await_ready, args=(tunnel_id, process, config), daemon=True
        ).start()
        return True
        ...
        try:
                    ...
                    if message:
                        with self._lock:
                            ...
                            if runtime and runtime.process is process:
                                ...
                                self._log(
                                    runtime, "info" if is_notice else "error", message
                                )
            code = process.wait()
        except (OSError, ValueError) as exc:
            if process.poll() is None and not self._terminate(process):
                with self._lock:
                    ...
                    if runtime and runtime.process is process:
                        self._set_error(
                            runtime, f"SSH 输出监控失败，且无法停止进程：{exc}"
                        )
                return
    ...
    def _await_ready(
        ...
        deadline = time.monotonic() + max(
            self.startup_timeout, config.connect_timeout + 2
        )
        while time.monotonic() < deadline:
            ...
            with self._lock:
                ...
                if (
                    ...
                    or runtime.state != TunnelState.CONNECTING
                ):
                    return
            ...
            if all(
                ...
                with self._lock:
                    ...
                    if (
                        ...
                        and runtime.state == TunnelState.CONNECTING
                    ):
                        runtime.state = TunnelState.CONNECTED
                        runtime.started_at = datetime.now()
                        ports = "、".join(
                            str(forward.local_port) for forward in forwards
                        )
                        self._log(
                            runtime, "success", f"隧道已连接，本地端口 {ports} 正在监听"
                        )
                return
            time.sleep(0.12)
        ...
        with self._lock:
            ...
            if (
                ...
                pending = [
                    ...
                    for forward in config.forwards
                    if not self._port_is_listening(
                        forward.bind_host, forward.local_port
                    )
                ]
    ...
    def stop(self, tunnel_id: str) -> bool:
        ...
        if not stopped:
            with self._lock:
                ...
                if runtime and runtime.process is process:
                    self._set_error(
                        runtime, "无法停止 SSH 进程，请在任务管理器中检查该进程"
                    )
        return stopped
    ...
    def _friendly_error(message: str, code: int) -> str:
        mappings = (
            ...
            (
                ("timeout, server", "connection reset by peer"),
                "SSH 连接已中断，请检查网络或保活设置",
            ),
        )
...
# Backward-compatible class name used by EasyTunnel 0.1.x integrations.
SSHManager = SSHTunnelService
