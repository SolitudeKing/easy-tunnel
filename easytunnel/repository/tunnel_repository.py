...
class TunnelRepository:
            ...
            for index, raw in enumerate(data["tunnels"]):
                ...
                if tunnel.identity_file:
                    ...
                    if not key_path.is_absolute():
                        tunnel.identity_file = str(
                            (self.path.parent / key_path).resolve()
                        )
    ...
    def example_tunnel(self) -> TunnelConfig:
        key = (
            str(self.sample_key.resolve())
            if self.sample_key and self.sample_key.is_file()
            else ""
        )
        return TunnelConfig(
            name="办公室远程桌面",
